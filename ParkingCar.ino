...
// used in RRT generation and path finding
struct vertex
{
  short x, y;
  short parentID;
...
{
    public:
      BLA::Matrix<3, 1> estimateState(const BLA::Matrix<3, 1>& cps, const BLA::Matrix<3, 1>& input, const BLA::Matrix<4, 1>& actualSensorValues);

    private:
      BLA::Matrix<3, 1> f(const BLA::Matrix<3, 1>& currentState, const BLA::Matrix<3, 1>& input);
      BLA::Matrix<4, 1> h(const BLA::Matrix<3, 1>& predictedState);
};
...
// Path functions
BLA::Matrix<3, 1> getGoalStateFromServer();
...
void rotateTo(float desiredAngle);
...
// Magnetometer bias values to bring mean of each measurement to 0.
float bias[3] = { -94.5, 100, -106 };

// Std deviations (bad variable names.. ) for the Q and R matrices
float varX = 0.4166;
float varY = 0.4166;
float varT = 0.03329;
const BLA::Matrix<3, 3> Q = { varX*varX, varX*varY, varX*varT,
                              varY*varX, varY*varY, varY*varT,
                              varT*varX, varT*varY, varT*varT };

// Std deviations here too
float varRf = 1.483;
float varRr = 1.691;
float varM  = 1.566;
const BLA::Matrix<4, 4> R = { varRf*varRf, varRf*varRr, varRf*varM, varRf*varM,
                              varRr*varRf, varRr*varRr, varRr*varM, varRr*varM,
                              varM*varRf,  varM*varRr,  varM*varM,  varM*varM,
                              varM*varRf,  varM*varRr,  varM*varM,  varM*varM,  };

// Current covariance and current state
BLA::Matrix<3, 1> currentState;
